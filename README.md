...
> **手指触控用不了，原因在平板固件，不在这个应用。**
>
> 平板在 DP-in 下会先认主机：认出 Mac（DP Alt Mode 的 SVID 报 Apple `0x05AC`）就**只输出笔**，触摸屏那一路数据连描述符都不创建。这发生在平板造出 USB 接口之前，Mac 上任何软件都改不了——不是「本应用暂未实现」，也没有能绕开的开关。
>
> 平板的系统日志自己写得很清楚：`usb_dp_relay: adapter_svid: 1452` → `host type: Mac` → `is_mac=1` → 只建笔的接口。同一块屏接 Windows 时手指是好的，因为走了另一个分支。
