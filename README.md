...
| **装上本工具**    | 笔尖轻点 = 鼠标左键；**快速划动 = 翻页滚动**（不误选文字）；**停住再划 = 拖拽**（拖窗口、划选文字） |
...
* **✋ 笔的四个手势** — **轻点** = 鼠标左键；**快速划动** = 滚轮翻页；**停住再划** = 拖拽（拖窗口、划选文字、拖滑块）；**停住不动** = 右键菜单（停在原地约 0.8 秒再抬手）。手势互不干扰：一手开始时是哪种，整手就按哪种走。
...
| 笔输入   | 滑动方式（滑动翻页 · 不选中文字 / 滑动选择 · 拖拽与划选）、**停住再划 = 拖拽 + 长按时间**、**长按不动 = 右键菜单**、滚动方向、滚动速度、**按键绑定（笔侧键 / 橡皮擦端 各绑一个动作）**、启用笔桥接、用笔的绝对坐标驱动光标（试验）、允许未验证的小米设备 |
