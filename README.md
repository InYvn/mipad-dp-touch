...
<p><i>不用装驱动、不用 Apple 授权、不改任何系统文件 —— 删掉 App 就等于卸载</i></p>
</div>
...
<p align="center">
  ...
  <br/>
  <sub>设置窗口（右上角是 logo）· 首次运行的分步授权：第 1 步没做完，第 2 步就是灰的</sub>
</p>
...
| **装上本工具**    | 笔尖轻点 = 鼠标左键；**快速划动 = 翻页滚动**（不误选文字）；**停住再划 = 拖拽**（拖窗口、划选文字）；速度和方向随你调 |
...
> **手指触控在 macOS 上没有可行方案。** 不是「我们没找到办法」：平板对外接主机只上报笔这一路数据，触摸屏那一路压根不存在，软件层面无处着手。所以本工具只解决笔，不承诺手指。

## ✨ 主要功能

* **🖊 轻点即点击** — 笔尖触碰 = 鼠标左键。落点取「按下瞬间」的位置，抬手时的手抖不会把点击带偏。

* **✋ 笔的三个手势** — **轻点** = 鼠标左键；**快速划动** = 滚轮翻页，全程一个鼠标按键都不发，所以**绝不会误选文字**（看文档 / 翻网页推荐）；**停住再划** = 拖拽（拖窗口、划选文字、拖滑块）。三个手势互不干扰：一手开始时是哪种，整手就按哪种走。
...
本工具**没有做 Apple 公证**（那要付费开发者账号），所以从网上下来的包第一次打开会被 Gatekeeper 拦一下 —— **不是文件坏了**。任选一种放行：
...
* 安装包本身约 **13 MB**（里面 App 约 25 MB），不依赖你机器上已装的 Python。
* 图标从仓库根目录的 `logo.png` 自动生成（≥1024×1024、带透明通道）—— 换图标 = 换这张图再打包。
...
| 笔输入   | 滑动方式（滑动翻页 · 不选中文字 / 滑动选择 · 拖拽与划选）、**停住再划 = 拖拽 + 长按时间**、滚动方向、滚动速度、启用笔桥接、用笔的绝对坐标驱动光标（试验）、允许未验证的小米设备 |
...
| 设备识别                           | ✅                                |
| **笔点击桥接**                      | ✅ 真机验证                           |
| **笔拖拽**（拖窗口、划选文字、拖滑块）          | ✅ 真机验证                           |
| **翻页滚动**（划动翻页，一个鼠标键都不发，绝不误选文字） | ✅ 真机验证                           |
| **停住再划 = 拖拽**（翻页模式下的第二个手势）     | ⏳ **真机待验** —— 新加的，等你用笔试          |
| **显示缩放**（只改平板那块屏，其他屏不动）        | ✅ 真机验证（1704×1136 ⇄ 1280×853 来回切） |
| **图形界面**（菜单栏 + 设置窗口 + 分步授权）    | ✅ 真机验证                           |
| 开机自启                           | ✅ 真机验证                           |
| 手指触控                           | ❌ **无可行方案**（设备不暴露触摸屏接口）          |
| 笔的右键 / 橡皮擦                     | ❌ 未做                             |
| 光标接管                           | ➖ 无必要（用系统原生光标更顺手，手指也没有数据可接管）     |
